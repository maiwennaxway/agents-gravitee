...
type SslInfo struct {
	...
	// Alias specified when you uploaded the cert and private key to the keystore. You must specify the alias name literally; you cannot use a reference. See <a href=\"https://docs.gravitee.com/api-platform/system-administration/options-configuring-tls\">Options for configuring TLS</a> for more.
	KeyAlias string `json:"keyAlias,omitempty"`
	// Name of the keystore on Edge. gravitee recommends that you use a reference to specify the keystore name so that you can change the keystore without having to restart Routers. See <a href=\"https://docs.gravitee.com/api-platform/system-administration/options-configuring-tls\">Options for configuring TLS</a> for more.
	...
	// Name of the truststore on Edge that contains the certificate or certificate chain used for two-way TLS. Required if `clientAuthEnabled` is `true`.  gravitee recommends that you use a reference to specify the truststore name so that you can change the truststore without having to restart Routers. See <a href=\"https://docs.gravitee.com/api-platform/system-administration/options-configuring-tls\">Options for configuring TLS</a> for more.
