...
func newPollStatsJob(options ...func(*pollgraviteeStats)) *pollgraviteeStats {
	job := &pollgraviteeStats{
		collector:     metric.GetMetricCollector(),
		mutex:         &sync.Mutex{},
		clonedProduct: make(map[string]string),
		dimension:     "apiproxy",
		logger:        log.NewFieldLogger().WithComponent("pollStatsJob").WithPackage("gravitee"),
	}
...
func withStartTime(startTime time.Time) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.startTime = startTime
	}
}

func withCacheClean() func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.cacheClean = true
	}
}

func withStatsClient(client definitions.StatsClient) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.client = client
	}
}

func withStatsCache(cache cache.Cache) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.statCache = cache
	}
}

func withCachePath(path string) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.cachePath = path
	}
}

func withIsReady(ready isReady) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.ready = ready
	}
}

func withAllTraffic(allTraffic bool) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.reportAllTraffic = allTraffic
	}
}

func withNotSetTraffic(notSetTraffic bool) func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
		p.reportNotSetTraffic = notSetTraffic
	}
}

func withProductMode() func(p *pollgraviteeStats) {
	return func(p *pollgraviteeStats) {
...
func (j *pollgraviteeStats) Ready() bool {
	return j.ready()
}

func (j *pollgraviteeStats) Status() error {
	return nil
}

func (j *pollgraviteeStats) Execute() error {
	id, _ := uuid.NewV4()
...
func (j *pollgraviteeStats) processMetricResponse(logger log.FieldLogger, metrics *models.Metrics) {
...
func (j *pollgraviteeStats) getBaseProduct(name string) string {
	if !j.isProduct {
	...
	for _, att := range prod.Attributes {
		if att.Name == gravitee.ClonedProdAttribute {
...
func (j *pollgraviteeStats) processMetric(logger log.FieldLogger, metData *metricData) {
...
func (j *pollgraviteeStats) cleanCache() {
