...
func (m mockProxyClient) GetAllProxies() (proxies gravitee.Proxies, err error) {
...
func (m mockProxyClient) GetRevisionConnectionType(proxyName, revision string) (*gravitee.HTTPProxyConnection, error) {
	return nil, nil
}
...
func (m mockProxyClient) GetSpecFromURL(url string, options ...gravitee.RequestOption) ([]byte, error) {
	assert.Equal(m.t, fullSpecPath, url)
	return []byte("spec"), nil
}

func (m mockProxyClient) GetRevisionPolicyByName(apiName, revision, policyName string) (policy *gravitee.PolicyDetail, err error) {
	assert.Contains(m.t, proxyName, apiName)
	assert.Contains(m.t, revName, revision)
	if policyName == apiKeyName {
		policy = &gravitee.PolicyDetail{
